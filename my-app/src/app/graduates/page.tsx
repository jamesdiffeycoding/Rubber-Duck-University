...
interface TopicAnswerData {
  topic: string;
  answer: string;
  date: string; // Assuming the date is stored as an ISO string
  comment?: string;
}

const DisplayStoredData = () => {
  // State to hold the data retrieved from localStorage
  const [storedData, setStoredData] = useState<TopicAnswerData[]>([]); // Correctly typed as an array of TopicAnswerData
  const [selectedCard, setSelectedCard] = useState<number | null>(null); // selectedCard can be a number or null
  const [editingComment, setEditingComment] = useState<string>(""); // editingComment should be a string
  const { isDarkMode } = useTheme(); // Assuming `isDarkMode` is a boolean
